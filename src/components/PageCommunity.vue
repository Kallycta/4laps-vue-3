...
export default {
  ...
  async mounted() {
    console.log(this.state.paginatedData);
  },
  methods: {
    ...
    addView(id) {
      ...
      console.log( this.state.community.QUESTIONS );
    },
    clickToHeaderSelect() {
    ...
    async getEducation(id) {
      console.log(id)
      const response = await fetch(`https://corp-st-dev.4lapy.ru/services/community/?community_id=${id}&action=getEducation`);
    ...
    async getCommunity(id) {
      try {
        const response = await fetch(`https://corp-st-dev.4lapy.ru${id}`);
        const data = await response.json();
...
<template>
    ...
    <h3 class="banner-header">Эксперты</h3>
    <div id="slider-experts" >
      <Carousel
          :itemsToShow="4"
          :itemsToScroll="1"
          :wrapAround="true"
        ...
        <Slide
          ...
          <div class="mobile-slider__slide item">
            <div class="slider-experts">
              <div class="slider-experts-link" @click="linkPersonalExpert()">
                ...
                <div class="slider-experts-container-img" :style="item.STYLE_STRING" >
                  <img :src='`${item.EMPLOYEE.PERSONAL_PHOTO ?  item.EMPLOYEE.PERSONAL_PHOTO : "https://expopk.ru/upload/iblock/51b/51b022afea429ec741eddb3050e51fdf.jpg"}`' alt="personal_photo" />
                </div>
              </div>
            </div>
          </div>
    ...
    <h3 class="banner-header" >Часто задаваемые вопросы</h3>

    <div class="community-expert__accordion-parent">
      <Component-Accordion
          v-for="(value, index) in state.paginatedData"
          v-bind:item="value"
          v-bind:index="index"
          v-bind:likes="value.LIKES"
          v-bind:alreadyLiked="value.ALREADY_LIKED"
          v-bind:views="value.VIEWS"
    ...
    <h3 class="banner-header">Обучающие материалы по породам</h3>
  <div v-if="state.educations?.type === 2">
              ...
              <div class="slider-kind-of-animal">
                <div class="slider-kind-of-animal-container-img">
                  <div class="slider-kind-of-animal-link" @click="linkKindOfAnimal(item.ID)">
                    <img src="https://png.pngtree.com/element_our/png/20180921/white-abstract-background-vector-png_110575.jpg" alt="kind_of_animal">
  ...
  <div v-if="state.educations?.type === 3">

  <div v-for="(value, name, index) in state.educations.items" v-bind:key="index" class="slider-kind-of-type-wrapper">
      <div class="community-expert__kind-of-animal">{{value.NAME}}</div>

      <div :id="`slider-kind-of-animal-${index}`" class="slider-kind-of-animal-id">
        <Carousel
            :itemsToShow="3"
            :itemsToScroll="1"
            :wrapAround="true"
  ...
  <Modal ref="modal" class="modal-community-expert">
        ...
        <div v-if="!showThanksMessage"  >
          <form class="form-modal">
            <div>
              ...
              <textarea  class="modal-textarea-no-resize" placeholder="Подскажите, чем нужно кормить щенка йоркширского терьера в возрасте 2 месяцев?" >
                </textarea>
            </div>
            <div>
    ...
    <template v-slot:footer>
      <div v-if="!showThanksMessage">
        <button class="bottom-container-btn btn-bottom-modal btn-modal-community-expert" v-on:click="openTnxModal()">
          Отправить вопрос
        </button>
      </div>
      <div v-if="showThanksMessage">
