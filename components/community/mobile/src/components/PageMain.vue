  ...
  <div id="mainTemplate">
    <div id="slider-main" class="main__slider">
      <div class="mobile-slider">
        <carousel :items-to-show="1" :itemsToScroll="1" :wrapAround="true">
            ...
            <div class="mobile-slider__slide item">
              <div class="banner">
                <img
                  ...
                  alt="banner_dog"
                  @click="routerLink(item.PROPERTY_LINK_VALUE, 'notRouter')"
                />
              </div>
            </div>
          </slide>
    ...
    <div id="slider-public">
      <carousel :items-to-show="1" :itemsToScroll="1" :wrapAround="true">
        <slide v-for="(item, index) in state.communities" v-bind:key="index">
          <div class="mobile-slider__slide item">
    ...
    <div id="slider-announce">
      <carousel
        :itemsToShow="
          state.announcements.length > 2 ? 2 : state.announcements.length
        "
        :itemsToScroll="1"
        :snapAlign="'start'"
        :wrapAround="true"
      >
        <slide v-for="(item, index) in state.announcements" v-bind:key="index">
          <div class="mobile-slider__slide item">
            <div
              class="banner-announce"
              @click="routerLink(`${item.LIST_PAGE_URL}`, 'not-router')"
            >
    ...
    <div class="bottom-section">
      <div class="bottom-section__text">
        Для обратной связи по работе сообщества пишите на почту отдела обучения
      </div>
    ...
    <div class="bottom-container">
      ...
      <Modal ref="modal" class="modal-community-expert">
        <template v-slot:header>
          <h3 v-if="!showThanksMessage">
            Чтобы стать экспертом, вам нужно ответить на несколько вопросов:
          </h3>
        ...
        <template v-slot:body>
          <div v-if="!showThanksMessage">
            <form @submit.prevent="submitQuestion()">
              <div>
                <label
                  >Являетесь ли вы специалистом, к которому обращаются за
                  советом и коллеги и покупатели? Если да, то кратко расскажите,
                  как вам удалось заслужить такое доверие?</label
                >
                <input required type="text" v-model="formData.question1" />
              </div>
              <div>
                <label
                  >Экспертом по содержанию какого вида питомца вы
                  являетесь?</label
                >
              ...
              <div>
                <label
                  >Есть ли у вас питомник? Если есть, то укажите данные
                  питомника или регистрационные данные питомцев?</label
                >
              ...
              <div>
                <label
                  >Был ли опыт посещения выставок и есть ли у ваших питомцев
                  оценка от эксперта?</label
                >
              ...
              <button
                class="bottom-container-btn btn-bottom-modal"
                type="submit"
              >
                Отправить
              </button>
        ...
        <template v-slot:footer>
          <div v-if="!showThanksMessage">
            ...
            <div class="bottom-container-text">
              *Вы можете указать вашу личную почту, ответ придется на вашу
              личную почту и почту магазина
            </div>
          </div>
          <div v-if="showThanksMessage">
            <button
      ...
      <div class="bottom-container-text">
        *заявка на подключение к экспертной группе доступна всем сотрудникам
        компании
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async getAnnouncements() {
      try {
        const response = await fetch(
          "/services/community/?action=getAnnouncements"
        );
        const data = await response.json();
