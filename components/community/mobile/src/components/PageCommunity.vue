...
export default {
  ...
  created() {
    try {
      ...
      this.getCommunity(
        `/services/community/?action=getCommunity&community_id=${id}`
      );
      this.getEducation(
        `/services/community/?action=getCommunity&community_id=${id}`
      );
  ...
  methods: {
    ...
    changeAmountLikes(id, action) {
      this.state.community.QUESTIONS = this.state.community.QUESTIONS.map(
        (item) => {
          if (item.ID === id) {
            if (action === "addLike") {
    ...
    async getEducation(id) {
      const response = await fetch(
        `/services/community/?community_id=${id}&action=getEducation`
      );
      const data = await response.json();
    ...
    async getEducationItems(id) {
      try {
        const response = await fetch(
          `/services/community/?action=getEducationItems&section_id=${id}`
        );
        const data = await response.json();
      } catch (e) {
        console.error(e);
    ...
    async getCommunities() {
      try {
        const response = await fetch(
          "/services/community/?action=getCommunities"
        );
        const data = await response.json();
          ...
          if (previous < 4) {
            switch (previous) {
              case 3:
                ...
                item.STYLE_STRING =
                  "background: radial-gradient(56.33% 62.84% at 35.21% 75.5%, #FEEF90 0%, rgba(254, 239, 144, 0.00) 60%),url('/local/templates/light_red/images/expert_bg4.png')";
                previous += 1;
                break;
              case 2:
                item.NUMBER_BACKGROUND = 3;
                item.STYLE_STRING =
                  "background: radial-gradient(56.33% 62.84% at 35.21% 75.5%, #F1D646 0%, rgba(253, 236, 142, 0.00) 60%),url('/local/templates/light_red/images/expert_bg3.png')";
                previous += 1;
                break;
              case 1:
                item.NUMBER_BACKGROUND = 2;
                item.STYLE_STRING =
                  "background: radial-gradient(56.33% 62.84% at 35.21% 75.5%, #EBB749 0%, rgba(235, 183, 73, 0.00) 60%),url('/local/templates/light_red/images/expert_bg2.png')";
                previous += 1;
                break;
              case 0:
                item.NUMBER_BACKGROUND = 1;
                item.STYLE_STRING =
                  "background: radial-gradient(56.33% 62.84% at 35.21% 75.5%, #F6B170 0%, rgba(255, 230, 202, 0.00) 60%),url('/local/templates/light_red/images/expert_bg1.png')";
                previous += 1;
                break;
            }
          } else {
    ...
    currentPages() {
      ...
      this.state.paginatedData = this.state.community.QUESTIONS.slice(
        indexStart,
        indexEnd
      );
    },
...
<template>
  <div class="community-expert__main-banner">
    <div class="community-expert__main-banner_text">
      {{ !state.community.UF_DETAIL_PICTURE ? state.community.NAME : "" }}
    </div>
  ...
  <div v-if="state.community.EXPERTS.length" id="slider-experts">
    <Carousel
      ...
      :wrapAround="true"
    >
      <Slide v-for="(item, i) in state.community.EXPERTS" v-bind:key="i">
        <div class="mobile-slider__slide item">
          <div class="slider-experts">
            <div
              class="slider-experts-link"
              @click="linkPersonalExpert(item.EMPLOYEE.LINK)"
            >
              <div class="slider-experts-name">
                <span
                  >{{ item.EMPLOYEE.NAME }} {{ item.EMPLOYEE.LAST_NAME }}</span
                >
              </div>
              <div class="slider-experts-profile">{{ item.NAME }}</div>
              <div
                class="slider-experts-container-img"
                :style="item.STYLE_STRING"
              >
                <img
                  :src="`${item.EMPLOYEE.PERSONAL_PHOTO}`"
                  alt="personal_photo"
                />
              </div>
            </div>
  ...
  <h3 v-if="state.paginatedData.length" class="banner-header">
    Часто задаваемые вопросы
  </h3>

  <div
    v-if="state.paginatedData.length"
    class="community-expert__accordion-parent"
  >
    <Component-Accordion
  ...
  <button
    id="show-modal"
    @click="openModal()"
    class="bottom-container-btn ask-us"
  >
    Задать вопрос
  </button>

  <h3 v-if="Object.keys(state.educations.items).length" class="banner-header">
    Обучающие материалы по породам
  </h3>
  <div
    v-if="
      state.educations?.type === 2 && Object.keys(state.educations.items).length
    "
  >
    <div class="slider-kind-of-type-wrapper">
      <div class="slider-kind-of-animal-id">
        <Carousel
          ...
          :wrapAround="true"
        >
          <Slide v-for="(item, i) in state.educations.items" v-bind:key="i">
            <div class="mobile-slider__slide item">
              <div class="slider-kind-of-animal">
                <div class="slider-kind-of-animal-container-img">
                  <div
                    class="slider-kind-of-animal-link"
                    @click="linkKindOfAnimal(item.ID)"
                  >
                    <img :src="item.PICTURE" alt="kind_of_animal" />
                    <div class="slider-kind-of-animal-link_text">
                      {{ item.NAME }}
                    </div>
                  </div>
                </div>
  ...
  <div
    v-if="
      state.educations?.type === 3 && Object.keys(state.educations.items).length
    "
  >
    <div
      v-for="(value, name, index) in state.educations.items"
      v-bind:key="index"
      class="slider-kind-of-type-wrapper"
    >
      <div
        v-if="value.subfolders?.length"
        class="community-expert__kind-of-animal"
      >
        {{ value.NAME }}
      </div>
      ...
      <div
        v-if="value.subfolders?.length"
        :id="`slider-kind-of-animal-${index}`"
        class="slider-kind-of-animal-id"
      >
        <Carousel
          :itemsToShow="1"
          :itemsToScroll="1"
          ...
          :wrapAround="true"
        >
          <Slide v-for="(item, i) in value.subfolders" v-bind:key="i">
            <div>
              <div>
                <div>
                  <div
  ...
  <Modal ref="modal" class="modal-community-expert">
        ...
        <form class="form-modal" @submit.prevent="askQuestion()">
          <div>
            ...
            <textarea
              ...
              class="modal-textarea-no-resize"
              placeholder="Подскажите, чем нужно кормить щенка йоркширского терьера в возрасте 2 месяцев?"
            >
            </textarea>
          </div>
          <div>
          ...
          <button
            type="submit"
            class="bottom-container-btn btn-bottom-modal btn-modal-community-expert"
          >
            Отправить вопрос
          </button>
          <div class="bottom-container-text">
            *Вы можете указать вашу личную почту, ответ придется на вашу личную
            почту и почту магазина
          </div>
        </form>
      </div>
      <div v-if="showThanksMessage">
        <div class="modal-text-tnx-message">
    ...
    <template v-slot:footer>
      ...
      <div v-if="showThanksMessage">
        <button
          class="bottom-container-btn btn-bottom-modal btn-modal-community-expert-tnx"
          v-on:click="closeTnxModal()"
        >
          Вернуться
        </button>
      </div>
